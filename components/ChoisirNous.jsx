...
export default function ChoisirNous() {
  return (
    <section id="choisir-nous" className="py-16 md:py-20 bg-gradient-to-b from-white to-[#FAF8F5]" suppressHydrationWarning>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-10">
          {reasons.map((reason, index) => (
            <motion.div
              ...
              className="group"
            >
              <div className="h-full bg-white rounded-2xl p-6 md:p-8 shadow-md hover:shadow-[0_0_15px_rgba(199,164,81,0.15)] transition-all duration-300 hover:scale-[1.02] border-2 border-[#C7A451]/20 hover:border-[#C7A451]/40 relative overflow-hidden">
                ...
                {/* Icon */}
                <div className={`inline-flex items-center justify-center w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-gradient-to-br ${reason.color} mb-5 group-hover:scale-105 transition-transform duration-300`}>
                  <reason.icon className="w-7 h-7 md:w-8 md:h-8 text-white stroke-[1.5]" />
                </div>

                {/* Title */}
                <h3 className="text-xl md:text-2xl font-semibold text-[#111] mb-3 group-hover:text-[#C7A451] transition-colors duration-200">
                  {reason.title}
                </h3>
